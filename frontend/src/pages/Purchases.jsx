...
import api from "../api";
import { Plus, Trash2, X, TruckIcon } from "lucide-react";

const today = () => new Date().toISOString().slice(0, 10);
const emptyForm = { material: "", supplier: "", quantity: "", rate: "", date: today(), notes: "" };
...
export default function Purchases() {
  ...
  const openNew = () => {
    setForm({ ...emptyForm, material: materials[0]?.id || "", supplier: suppliers[0]?.id || "" });
    setShowForm(true);
  };
  ...
  const submit = async (e) => {
    e.preventDefault();
    await api.post("purchases/", { ...form, supplier: form.supplier || null });
    setShowForm(false);
    load();
  };

  const remove = async (id) => {
    if (!confirm("Delete this purchase entry? Stock will be adjusted back.")) return;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-2xl font-extrabold text-[#24272B] flex items-center gap-2">
            <TruckIcon size={22} className="text-[#37515E]" /> Stock In — Purchases
          </h1>
          <p className="text-sm text-[#71767a] mt-1">Record material bought from suppliers. Stock quantity updates automatically.</p>
        </div>
        <button onClick={openNew} className="flex items-center gap-2 bg-[#37515E] text-white text-sm font-semibold px-4 py-2.5 rounded-md hover:bg-[#24272B] transition-colors">
      ...
      {showForm && (
        <form onSubmit={submit} className="bg-[#F7F5F0] border border-[#DAD5C9] rounded-lg p-5 mb-6 animate-ticket-in">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-display font-bold text-sm">New Purchase Entry</h3>
          ...
          <tbody>
            {!loading && rows.map((r) => (
              <tr key={r.id} className="border-t border-[#DAD5C9]">
                ...
                <td className="px-4 py-3 text-right font-mono-data font-semibold text-[#37515E]">Rs {Number(r.total_amount).toLocaleString()}</td>
